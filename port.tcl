...
## or Timeout. It also portscans users on join and can ban the user and/or sendan op notice if ##
...
set portcheck_setting(cmd_pub2) "!port"

###############################################################
# Set the dcc command for checking the status of a port here. #
###############################################################

set portcheck_setting(cmd_dcc) "port"
...
# further settings) Scanning for a channel is enabled via              #
...
bind pub $portcheck_setting(flag) $portcheck_setting(cmd_pub2) portcheck_scan_pub
...
proc portcheck_scan_pub {nick uhost hand chan text} {
  ...
  if {$port == ""} {
    ...
    if {[catch {set sock [socket -async $host $port]} error]} {
      putquick "PRIVMSG $chan :Connection to \00312$host\003 \(\00304$port\003\) was refused."
      } else {
...
proc portcheck_connected_pub {chan sock host port timerid} {
  ...
  if {[set error [fconfigure $sock -error]] != ""} {
    close $sock
    putquick "PRIVMSG $chan :Connection to \00312$host\003 \(\00304$port\003\) failed. \([string totitle $error]\)"
    } else {
    fileevent $sock writable {}
    fileevent $sock readable [list portcheck_read_pub $chan $sock $host $port]
    putquick "PRIVMSG $chan :Connection to \00312$host\003 \(\00304$port\003\) accepted."
  }
}
proc portcheck_timeout_pub {chan sock host port} {
  close $sock
  putquick "PRIVMSG $chan :Connection to \00312$host\003 \(\00304$port\003\) timed out."
}
